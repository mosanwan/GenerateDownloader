...
class DownloaderThread(threading.Thread):
	...
	def start_download(self):
		...
		headers = {'Range': 'bytes=%d-' % self.current_mission.content_download}
		with closing(requests.get(self.current_mission.url, stream=True, headers=headers)) as response:
			if self.current_mission.content_length == -1:
			...
			try:
				with open(self.current_mission.file_path + ".temp", "wb") as file:
					file.seek(self.current_mission.content_download, 0)
